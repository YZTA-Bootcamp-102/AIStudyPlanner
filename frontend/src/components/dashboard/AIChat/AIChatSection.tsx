...
import { getMe } from '../../../services/auth'; // Bu yolu kendi projene göre kontrol et
...
const AIChatSection = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: '', // bunu aşağıda kullanıcı geldikten sonra ayarlayacağız
      isAI: true,
      timestamp: new Date()
    },
  ]);
  ...
  // Kullanıcıyı getir ve ilk mesajı username ile ayarla
  useEffect(() => {
    getMe()
      .then((userData) => {
        setUser(userData);
        setMessages((prev) => {
          // İlk mesajı username eklenmiş hale güncelle
          const newMessages = [...prev];
          newMessages[0].text = `Merhaba ${userData.username}! Ben senin AI eğitim asistanınım! 🎓
...
Size nasıl yardımcı olabilirim? ✨`;
      ...
      .catch((err) => {
        ...
        setUser(null);
        // Kullanıcı gelmezse default mesajı koyabilirsin:
        setMessages((prev) => {
          const newMessages = [...prev];
          newMessages[0].text = `Merhaba Kullanıcı! Ben senin AI eğitim asistanınım! 🎓
...
Size nasıl yardımcı olabilirim? ✨`;
  ...
  // Scroll otomatik ayarlandı zaten senin kodda
        ...
        <div className="flex items-center space-x-3">
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder="Merhaba! AI asistanıma ne sormak istersin? 😊"
              rows={3}
              className="w-full px-5 py-4 rounded-3xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white text-base placeholder-gray-400 dark:placeholder-gray-500 resize-none
                focus:outline-none focus:ring-2 focus:ring-orange-500/60 focus:border-orange-500 transition-shadow duration-300 shadow-sm hover:shadow-md"
            />
            <MessageCircle size={20} className="absolute right-4 top-5 text-orange-400 dark:text-orange-500 opacity-70 pointer-events-none" />
          </div>
          <button
            onClick={sendMessage}
            disabled={!chatMessage.trim()}
            className="p-4 rounded-3xl bg-gradient-to-br from-orange-500 to-orange-600 text-white transition-all duration-300 flex-shrink-0 shadow-lg hover:shadow-orange-500/40 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:transform-none"
