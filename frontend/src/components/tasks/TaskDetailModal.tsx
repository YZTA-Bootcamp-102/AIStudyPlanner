...
import type { Subtask, Comment, CategoryType, PriorityType, RepeatType } from '../../types/calendar';

const initialCategories = ['study', 'exam', 'project', 'homework', 'lab', 'language', 'reading'] as CategoryType[];
const priorities = [
  { value: 'low' as PriorityType, label: 'Düşük', color: 'bg-emerald-500', hoverColor: 'hover:bg-emerald-100 dark:hover:bg-emerald-900/30', textColor: 'text-emerald-500' },
  { value: 'medium' as PriorityType, label: 'Orta', color: 'bg-amber-500', hoverColor: 'hover:bg-amber-100 dark:hover:bg-amber-900/30', textColor: 'text-amber-500' },
  { value: 'high' as PriorityType, label: 'Yüksek', color: 'bg-rose-500', hoverColor: 'hover:bg-rose-100 dark:hover:bg-rose-900/30', textColor: 'text-rose-500' }
];

const repeatOptions = [
  { value: 'none' as RepeatType, label: 'Tekrarlanmıyor', icon: X },
  { value: 'daily' as RepeatType, label: 'Her Gün', icon: Calendar },
  { value: 'weekly' as RepeatType, label: 'Her Hafta', icon: Calendar },
  { value: 'monthly' as RepeatType, label: 'Her Ay', icon: Calendar },
  { value: 'yearly' as RepeatType, label: 'Her Yıl', icon: Calendar }
];
...
const TaskDetailModal = ({ isOpen, onClose, task = initialTask, onSave, onDelete }: TaskDetailModalProps) => {
  ...
  const handleAddSubtask = (e?: React.MouseEvent) => {
    ...
    if (newSubtask.trim()) {
      ...
      const newForm = {
        ...form,
        subtasks: [...(form.subtasks || []), newSubtaskItem]
      };
      setForm(newForm);
      debouncedSave(newForm, false);
      setNewSubtask('');
  ...
  const handleAddCategory = () => {
    if (newCategory.trim() && !categories.includes(newCategory as CategoryType)) {
      setCategories([...categories, newCategory as CategoryType]);
      handleChange('category', newCategory as CategoryType);
      setNewCategory('');
      setShowCategorySelect(false);
    }
  };
  ...
  const handleAddComment = () => {
    if (newComment.trim()) {
      const newCommentItem: Comment = {
        ...
        userId: 'current-user' // This should come from auth context in a real app
      };
      const newForm = {
        ...form,
        comments: [...(form.comments || []), newCommentItem]
      };
      setForm(newForm);
      debouncedSave(newForm, false);
      setNewComment('');
  ...
  const handleCategorySelect = (category: CategoryType, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    handleChange('category', category);
  };

  const handlePrioritySelect = (priority: PriorityType, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-40 flex items-center justify-center pointer-events-none p-4">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-4xl relative animate-fade-in flex flex-col md:flex-row overflow-hidden pointer-events-auto" onClick={e => e.stopPropagation()}>
          {/* Left panel content */}
          <div className="flex-1 p-0 relative">
            {/* Header */}
            <div className="sticky top-0 z-[60] flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
              ...
              {showMenu && (
                <div className="absolute right-16 top-16 w-48 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 py-1 z-[70]">
                  <button
                    onClick={onDelete}
              ...
              <div className="mb-8 bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-700/50">
                ...
                <div className="space-y-2">
                  {form.subtasks.map((subtask, idx) => (
                    <div key={subtask.id} className="group flex items-center gap-3 p-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 hover:border-orange-500/50 transition-colors">
                      <button
              ...
              <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-700/50">
                ...
                <div className="space-y-4">
                  {form.comments.map((comment) => (
                    <div key={comment.id} className="flex items-start gap-3 p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700/50 group">
                      <div className="w-8 h-8 rounded-full bg-orange-500 text-white flex items-center justify-center font-medium">
                      ...
                      <div className="flex-1">
                        <textarea
                          ...
                          onChange={e => {
                            ...
                            if (idx !== -1) {
                              newComments[idx] = { ...comment, text: e.target.value };
                          ...
                          rows={Math.min(5, (comment.text?.split('\n').length || 1))}
                        />
                      ...
                      <button
                        onClick={() => {
                          const newComments = form.comments.filter(c => c.id !== comment.id);
                          handleChange('comments', newComments);
                        }}
                        className="opacity-0 group-hover:opacity-100 p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full transition-all"
                      >
          ...
          <div className="w-full md:w-80 bg-gray-50 dark:bg-gray-800 border-l border-gray-100 dark:border-gray-700 p-8 relative flex flex-col gap-6 rounded-r-2xl">
            ...
            <div className="space-y-2" ref={categorySelectRef}>
              ...
              <div className="relative">
                ...
                {showCategorySelect && (
                  <div className="absolute top-full left-0 mt-2 w-full bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 py-2 z-[70]">
            ...
            <div className="space-y-2" ref={datePickerRef}>
              ...
              <div className="relative">
                ...
                {showDatePicker && (
                  <div className="absolute top-full left-0 mt-2 w-full bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-4 z-[70]">
                    <div className="space-y-4">
